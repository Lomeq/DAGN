...
def main():
    ...
    if (
        os.path.exists(training_args.output_dir)   #存在output_dir路径
        and os.listdir(training_args.output_dir)   #如果该路径下存在文件
        and training_args.do_train
        and not training_args.overwrite_output_dir   #没有指定覆盖原来的output
    ):
        raise ValueError(
    ...
    def compute_metrics(p: EvalPrediction) -> Dict:
        preds = np.argmax(p.predictions, axis=1)   #选取四个标签中概率最大的
    ...
    if training_args.do_eval:
        ...
        if trainer.is_world_master():   #是主线程又怎么了？多线程运行规则是怎么样的
            with open(output_eval_file, "w") as writer:
                ...
                results.update(result)  #merge two dictionaries
